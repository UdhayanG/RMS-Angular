...
export class RegistrationComponent implements OnInit {
	...
	submitFormPhone() {
		...
		this.userService.registerUser(this.phoneform.getRawValue()).subscribe(data => {
			...
			if ("ErrStatus" in data) {
				...
				else if (JSON.stringify(data["RequestID"])!="" ||JSON.stringify(data["RequestID"])!=null  ) {
					this.phoneform.reset();
	...
	async loginWithFacebook():  Promise<any> {
		this.user = await this.authService.signIn(FacebookLoginProvider.PROVIDER_ID);
		//this.authService.authState.subscribe((user) => {
		//	this.user = user;
		// });
		console.log("fb ===========>",JSON.stringify(this.user));
		this.userService.socialRegister(this.user).subscribe(data => {
			console.log(data);
			this.router.navigate(['Profile']);
		});
